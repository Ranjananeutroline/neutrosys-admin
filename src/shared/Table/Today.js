...
function Upcoming({ childAddAppointment, searchText, searchTermFromUpcoming }) {
    ...
    const fetchData = async () => {
      try {
        ...
        ? allAppointments.filter((item) => {
            ...
            console.log(formattedDateBooking);
      
            console.log(appointmentDate);
            console.log(currentData);
            return formattedDateBooking === formattedDateToday;
          })
        : [];
        ...
        const formattedData = filteredAppointments?.map((item, index) => ({
          ...
          reminder: "Notify",
          toNotify: item.toNotify ? "Notified" : "Notify",
        }));
  ...
  const handleNotifyClick = async (item) => {
    // console.log(item.itemId, item.email);
    // const itemId = item.itemId;
    // const email = item.email;
    dispatch(notifyAppointmentHere(item));
  };
          ...
          {data?.map((item) => (
            ...
            >
              ...
              <td className="  flex gap-2 flex-col md:flex-row justify-center items-center    md:h-[60px] ">
                ...
                <button
                  onClick={() => onOpenRescheduleModal(item.email, item.docid)}
                  className="group relative"
                >
                  <img
                    className="md:hidden w-[13px] h-[15px]"
        ...
        {data.map((item) => (
          ...
          >
            <div className=" relative  w-full">
              ...
              <div className=" bg-[white] flex flex-col  rounded-[10px]  justify-between gap-3 bg-div">
                <p
